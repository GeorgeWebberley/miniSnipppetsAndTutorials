...
void recursive(char *start, char *end, FILE *file)
{
  char candidate[WORDLENGTH];
  int distance;
  distance = edit_distance(start, end);
  if(distance == 0){
    return;
  }
  rewind(file);
  while(fscanf(file, "%s", candidate) != EOF){
    if((edit_distance(candidate, end) == (distance - 1)) && (edit_distance(candidate, start) == 1)){
      fprintf(stdout, "%s\n", candidate);
      recursive(candidate, end, file);
    }
  }
}
