...
void changeArray(char current[ROWS][COLS], char next[ROWS][COLS])
{
  ...
  for(row = 0; row < ROWS; row++){
    for(column = 0; column < COLS; column++){
      if(convertLetter(current, row, column) == 0){
...
int convertLetter(char grid[ROWS][COLS], int y, int x)
{
  ...
  /* Inlcuded for testing and error checking. This should not be reached
  (as already checked by the convertToArray function). */
  return 0;
}
